...
def check_passport(birth_date, issue_date):
    ...
    # Valid if issued after 14 and age is between 14 and 20 plus 1 month
    if age_m >= 14 * 12 and age_m < 20 * 12 + 1 and issue_d >= birth_d.replace(year=birth_d.year + 14):
        return True

    # Valid if issued after 20 and age is between 20 + 1 month
    # and 45 + 1 month
    if age_m >= 20 * 12 + 1 and age_m < 45 * 12 + 1 and issue_d > birth_d.replace(year=birth_d.year + 20):
        return True

    # Valid if issued after 45 and age is after 45 + 1 month
